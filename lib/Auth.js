...
class Auth {
  ...
  static decodeToken (token = null, complete = false) {
    if (!token || token === null) {
      return Promise.reject(Boom.badRequest('invalid encoded jwt'))
    }
    const decoded = jwt.decode(token, { complete: complete })
    if (!decoded || decoded === null) {
      return Promise.reject(Boom.badRequest('unable to decode token'))
    }
  ...
  static verifyToken (token = null, signSecret = null, options = {}) {
    if (!token || token === null) {
      return Promise.reject(Boom.badRequest('missing or invalid token'))
    }
  ...
  static signToken (payload = null, signSecret = null, options = {}) {
    if (!payload || payload === null) {
      return Promise.reject(Boom.badRequest('invalid payload supplied'))
    }
    return Signer.isValidSignSecret(signSecret).then(() => {
